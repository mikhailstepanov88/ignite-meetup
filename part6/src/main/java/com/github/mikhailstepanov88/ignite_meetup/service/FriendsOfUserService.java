...
public class FriendsOfUserService {
    ...
    public Optional<Long> createFriendOfUser(long userId, long friendId) {
        return repository.executeInTransaction(OPTIMISTIC, SERIALIZABLE, () -> {
            if (!repository.createFriendOfPerson(userId, friendId))
                return Optional.empty();
            if (!repository.createFriendOfPerson(friendId, userId))
                return Optional.empty();
    ...
    public boolean deleteFriendOfUser(long userId, long friendId) {
        return repository.executeInTransaction(OPTIMISTIC, SERIALIZABLE, () -> {
            if (!repository.deleteFriendOfPerson(userId, friendId))
                return Optional.empty();
            if (!repository.deleteFriendOfPerson(friendId, userId))
                return Optional.empty();
